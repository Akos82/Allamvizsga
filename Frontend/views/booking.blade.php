...
<div class="grid lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-4">
        <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex flex-wrap gap-2 mt-2">
                                ...
                                <span class="inline-flex items-center gap-1 text-xs rounded-full px-2.5 py-0.5
                                    ...
                                    {{ $b->with_presentation ? 'Előadással (300 lej)' : 'Előadás nélkül (250 lej)' }}
                                </span>
                                @if($b->show)
                                ...
                                <span class="inline-flex items-center gap-1 text-xs rounded-full px-2.5 py-0.5
                                    ...
                                    {{ $b->status === 'confirmed' ? 'Megerősített' : 'Függőben' }}
                                </span>
                            </div>
                            @if($b->notes)
            ...
            <div class="text-center py-10 text-gray-400">
                ...
                <p class="text-sm">Jelenleg nincs aktív foglalás.</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="space-y-5">
        <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
            <h3 class="font-bold text-gray-800 mb-4 text-sm">Foglaltsági naptár</h3>
            ...
            <div class="grid grid-cols-7 mb-1">
                @foreach(['H','K','Sz','Cs','P','Szo','V'] as $d)
            ...
            <div class="flex flex-col gap-1.5 mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500">
                <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-brand-500 inline-block"></span>Szabad</span>
                <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-amber-400 inline-block"></span>Részben foglalt</span>
                <span class="flex items-center gap-1.5"><span class="w-2.5 h-2.5 rounded-full bg-red-400 inline-block"></span>Teljesen foglalt</span>
            </div>
        </div>
        ...
        <a href="{{ url('/admin/bookings') }}"
           class="flex items-center justify-center gap-2 w-full bg-brand-600 hover:bg-brand-700 text-white font-semibold py-3 rounded-xl transition text-sm">
            ...
            Összes foglalás kezelése (Admin)
        </a>
    </div>
</div>
@else
{{-- ===== FELHASZNÁLÓ NÉZET ===== --}}
        ...
        <div class="grid lg:grid-cols-3 gap-8">
                ...
                <form method="POST" action="{{ route('booking.store') }}" id="booking-form">
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="grid md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Teljes név <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">E-mail cím <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Telefonszám <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Intézmény / Szervezet <span class="text-red-500">*</span></label>
                                <input type="text" name="group_name" value="{{ old('group_name') }}" required
                                       placeholder="pl. Petőfi Sándor Gimnázium, 9-10. évfolyam"
                                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
                                <p class="text-xs text-gray-400 mt-1">Adja meg az iskola vagy szervezet nevét és a látogatók korosztályát is.</p>
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="grid md:grid-cols-2 gap-6">
                            ...
                            {{-- Dátum --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Látogatás napja <span class="text-red-500">*</span></label>
                                <input type="date" name="booking_date" value="{{ old('booking_date') }}" required
                                       min="{{ date('Y-m-d') }}"
                                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
                                <p class="text-xs text-gray-400 mt-1">Minden esetben visszajelzünk a választott időpontra!</p>
                                @error('booking_date')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                            </div>

                            {{-- Létszám --}}
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Hány fővel tervezi a látogatást? <span class="text-red-500">*</span></label>
                                <input type="number" name="group_size" value="{{ old('group_size') }}" required min="1" max="27"
                                       class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition">
                                <p class="text-xs text-gray-400 mt-1">A planetárium kapacitása <strong>maximum 27 fő</strong>.</p>
                        ...
                        {{-- Időpont --}}
                        <div class="mt-5">
                            <label class="block text-sm font-medium text-gray-700 mb-3">Hány órától szeretne meglátogatni? <span class="text-red-500">*</span></label>
                            <p class="text-xs text-gray-500 mb-3">Minden esetben visszajelzünk a választott időpontra! <strong>SAPIENTIÁS csoportok KIZÁRÓLAG 16:00 óra utáni időpontot tudnak választani.</strong></p>
                            ...
                            <div id="other-time-box" class="{{ old('booking_time') && !in_array(old('booking_time'), ['08:00','09:00','10:00','11:00','12:00','16:00','17:00','18:00']) ? '' : 'hidden' }} mt-3">
                                ...
                                <p class="text-xs text-gray-400 mt-1">Adja meg a kívánt időpontot.</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            <label class="flex items-start gap-4 border-2 rounded-xl p-4 cursor-pointer transition
                                ...
                                <div>
                                    <p class="font-semibold text-gray-800">Előadással</p>
                                    <p class="text-2xl font-bold text-brand-600 mt-1">300 lej</p>
                                    <p class="text-xs text-gray-500 mt-1">Szakmai előadóval kísért vetítés</p>
                                </div>
                            </label>
                            <label class="flex items-start gap-4 border-2 rounded-xl p-4 cursor-pointer transition
                                ...
                                <div>
                                    <p class="font-semibold text-gray-800">Előadás nélkül</p>
                                    <p class="text-2xl font-bold text-brand-600 mt-1">250 lej</p>
                                    <p class="text-xs text-gray-500 mt-1">Önálló film-vetítés</p>
                                </div>
                            </label>
                        </div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        <h2 class="text-lg font-bold text-gray-800 mb-2 flex items-center gap-2">
                            ...
                            Előadás kiválasztása
                        </h2>
                        <p class="text-sm text-gray-500 mb-5">Válasszon egyet az aktív előadásainkból.</p>
                            ...
                            <div class="show-card relative border-2 rounded-xl cursor-pointer transition select-none
                                ...
                                <div class="flex items-center gap-3 p-3 min-h-[72px]">
                                    ...
                                    <div class="min-w-0 pr-5">
                                        ...
                                        <p class="text-xs text-gray-400 mt-0.5">{{ $show->duration_minutes }} perc</p>
                                    </div>
                                </div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                        <h2 class="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                            ...
                            Megjegyzés
                            <span class="text-sm font-normal text-gray-400 ml-1">(opcionális)</span>
                        </h2>
                        <textarea name="notes" rows="3" placeholder="Egyéb kérések, megjegyzések..."
            ...
            <div class="space-y-5">
                ...
                {{-- Naptár --}}
                <div class="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
                    <h3 class="font-bold text-gray-800 mb-4 text-sm">Szabad időpontok</h3>
                    <div class="flex items-center justify-between mb-3">
                        <button id="prev-month" class="w-8 h-8 rounded-lg border border-gray-200 hover:bg-gray-50 flex items-center justify-center transition text-gray-500">
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                        </button>
                ...
                <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 text-sm text-gray-500">
                    <p class="font-semibold text-gray-700 mb-2">Fontos tudnivalók</p>
                    <ul class="space-y-1.5">
                        <li class="flex gap-2"><span class="text-brand-500">•</span>Maximum 27 fő/csoport</li>
                        <li class="flex gap-2"><span class="text-brand-500">•</span>Nyitvatartás: H–P 8:00–19:00</li>
                        <li class="flex gap-2"><span class="text-brand-500">•</span>Hétvégén zárva</li>
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', function(e) {
            ...
            if (otherRadio && otherRadio.checked) {
                ...
                if (!otherVal) {
                    e.preventDefault();
                    alert('Kérem adja meg az egyéb időpontot!');
                    return;
                }
    ...
    if (bookedDates[dateStr] && bookedDates[dateStr].length) {
        div.innerHTML = bookedDates[dateStr].map(b =>
            ...
            ? `<div class="flex gap-2 items-start p-2 bg-red-50 border border-red-100 rounded-lg text-xs">
                ...
                <div><span class="font-bold">${b.time.substring(0,5)}</span> – ${b.group} <span class="text-gray-400">(${b.group_size} fő)</span></div>
               </div>`
            : `<div class="flex gap-2 items-center p-2 bg-red-50 border border-red-100 rounded-lg text-xs">
                ...
                <span class="font-medium">${b.time.substring(0,5)}</span> – Foglalt (${b.group_size} fő)
               </div>`
        ).join('');
    } else {
        div.innerHTML = `<p class="text-brand-600 text-xs font-medium flex items-center gap-1"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>Szabad nap – nincs foglalás</p>`;
    }
